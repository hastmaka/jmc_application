...
export default function CaregiverCompetenceModal({id}) {
    const {
        resetState,
        handleInput,
        errors,
    ...
    useLayoutEffect(() => {modalData('rbt_caregiver_competence', id, 4).then()}, [id])

    const handleCancel = () => {
        resetState()
        closeModal('add_caregiver_competence')
    }
        ...
        if (checkRequired('skill_behavior', FIELDS)) {
            await toast.U({
                ...
                id: {
                    ...
                    message: `${id ? 'Updating' : `Saving`} caregiver competence...`
                },
                update: {
                    success: `Caregiver Competence ${id ? 'updated' : 'created'} successfully`,
                    error: `Caregiver Competence couldn't be ${id ? 'update' : 'create'}`
                },
                cb: async () => handleSubmitCompetence(4)
            })
            resetState()
            closeModal('add_caregiver_competence')
        }
    ...
    return (
        <Stack>
            <Flex gap={16} flex={1}>
                ...
                {modal.state === 'create'
                    ? <CreateTabs signal={CompetenceController} task who={4}/>
                    : <EditTabs signal={CompetenceController} task who={4}/>
                }
...
CaregiverCompetenceModal.propTypes = {
    id: PropTypes.number
}
