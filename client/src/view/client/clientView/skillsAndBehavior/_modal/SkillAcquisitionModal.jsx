...
export default function SkillAcquisitionModal({id}) {
    const {
        resetState,
        handleInput,
        errors,
    ...
    useLayoutEffect(() => {modalData('skill_behavior', id, 3).then()}, [id])

    const handleCancel = () => {
        resetState()
        closeModal('skill_acquisition')
    }
        ...
        if (checkRequired('skill_behavior', FIELDS)) {
            await toast.U({
                ...
                id: {
                    ...
                    message: `${id ? 'Updating' : `Saving`} skill acquisition...`
                },
                update: {
                    success: `Skill acquisition ${id ? 'updated' : 'created'} successfully`,
                    error: `Skill acquisition couldn't be ${id ? 'update' : 'create'}`
                },
                cb: async () => handleSubmitSkillBehavior(3)
            })
            closeModal('skill_acquisition')
            resetState()
        }
    ...
    return (
        <Stack>
            <Flex gap={16} flex={1}>
                        ...
                        handleInput={(name, value, api) => {
                            ...
                            if (name === 'skill_behavior_measure' && value && modal.state === 'edit') {
                                ...
                                if (value !== local.tempSkillBehaviorMeasure) {
                                    return openModal({
                                        modalId: 'update_skill_acquisition',
                                        title: 'Confirm Action',
                                        size: 'sm',
                                        children: (
                                            <SaveChangeModal
                                                message='You are about to change the measure, this will reset Measure'
                                                handleCancel={() => closeModal('update_skill_acquisition')}
                                                handleSave={async () => {
                ...
                {modal.state === 'create'
                    ? <CreateTabs signal={SkillsAndBehaviorController} who={3}/>
                    : <EditTabs signal={SkillsAndBehaviorController} who={3}/>
                }
...
SkillAcquisitionModal.propTypes = {
    id: PropTypes.number
}
