...
export default function EmployeeDocumentStatus() {
    const {
        employeeDocumentStatusGetData,
        employeeDocumentStatusData,
        employeeDocumentStatusLoading
    } = EmployeeInfoController

    useLayoutEffect(() => {
        employeeDocumentStatusGetData().then();
        return () => {
            EmployeeInfoController.employeeDocumentStatusData = []
            EmployeeInfoController.employeeDocumentStatusLoading = true
        }
    }, []);
    ...
    if (employeeDocumentStatusLoading) return <Skeleton mih={346} radius='sm'/>

    return (
        <EzCard title='Document Status'>
            {employeeDocumentStatusData.length > 0 ? employeeDocumentStatusData.map((address: any, index: number) => (
                <CardItem
